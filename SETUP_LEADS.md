...
### (Опционально) AI-диагностика дашборда — `/api/insight`

Кнопка **🤖 AI-диагностика** в дашборде. Чтобы включить AI-режим (модель
`claude-haiku-4-5`), добавь ещё одну переменную:

| Name | Value |
|------|-------|
| `ANTHROPIC_API_KEY` | `sk-ant-…` (из <https://console.anthropic.com/settings/keys>) |

Без неё эндпоинт автоматически отдаёт детерминированный диагноз по правилам —
фича работает и без ключа, просто без LLM-формулировок.
